...
def insert_data():
    ...
    if request.method == 'POST':
        ...
        #if rev_star == '':
            #rev_star = 'NULL'
        urgency = predict(title)
        if urgency == '0':
            urgent = "FALSE"
        else:
            urgent = "TRUE" 
        cursor = cnx.cursor()
        cursor.execute("INSERT INTO report (title,created_at,category,longi,lat,photo,urgent) VALUES ('{}',NOW(),'{}',{},{},'{}',{})".format(title,cat,longi,lat,photo_url,urgent))
        cnx.commit()
...
def predict(x):
    ...
    df1 = pd.read_excel('JakRespons-Sample.xlsx')
